...
///
/// `start()` and `wait()` are synchronous: they are invoked once at process
...
/// `withTaskExecutorPreference`. The sync surface area here is bounded to
/// process bootstrap.
public final class StarlightApp: @unchecked Sendable {
    ...
    /// every per-loop listener is bound; call `wait()` afterwards to block
    /// the calling thread while event loops process connections.
    @inlinable
    public func start(host: String = "0.0.0.0", port: Int = 8080) throws {
        try self.server.start(host: host, port: port)
    }

    /// Block the calling thread (typically `main`) until the server stops.
    /// Use this at the top level of your executable to keep the process
    /// alive while connections are handled concurrently on the event loops.
    @inlinable
    public func wait() throws {
        try self.server.wait()
    }

    /// Convenience: `start()` + `wait()`. Synchronous — meant to be the last
    /// line of a `@main` `static func main() throws`.
    @inlinable
    public func run(host: String = "0.0.0.0", port: Int = 8080) throws {
        try self.start(host: host, port: port)
        try self.wait()
    }
}
